...
```{r}
#| label: fig-parcels-ga
#| fig-cap: "Total acres by Sentate District"
#| eval: false

# There are too many house districts to make a decent bar chart. It would be best
# to create a choropleth map of the House Districts.
...
ggplot(faith_summary_ga,
       aes(x = reorder(house, total_acres),
           y = total_acres)) +
  geom_point(fill = "#011E41") +
  geom_text(
    aes(label = scales::comma(round(total_acres, 0))), 
    color = "#011E41", 
    hjust = -0.2
  ...
  theme_hfv() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) +
  labs(title = "Total acres of faith-based owned land by House District",
       caption = "**Source:** HousingForward Virginia & LOVELAND Technologies, Regrid 2024 Dataset.")
